...
from coda.apps.opencost.transformers import to_publication
...
def test__publication_with_invoice__transforming_to_opencost__cost_data_includes_invoice_with_correct_position_data() -> (
    ...
    creditor = Creditor.objects.create(name="Invoice Creditor")
    invoice = Invoice.objects.create(
        creditor=creditor, date=date(2024, 6, 1), number="INV-2024-001", status="paid"
    )
...
def test__publication_with_invoice_multiple_positions__transforming_to_opencost__amount_invoice_and_amounts_paid_are_correct() -> (
    ...
    creditor = Creditor.objects.create(name="Invoice Creditor")
    invoice = Invoice.objects.create(
        creditor=creditor, date=date(2024, 6, 1), number="INV-2024-002", status="paid"
    )
...
def test__publication_with_multiple_invoices__transforming_to_opencost__all_invoices_are_included() -> (
    ...
    creditor1 = Creditor.objects.create(name="First Creditor")
    invoice1 = Invoice.objects.create(
        creditor=creditor1, date=date(2024, 5, 1), number="INV-2024-101", status="paid"
    )
    ...
    creditor2 = Creditor.objects.create(name="Second Creditor")
    invoice2 = Invoice.objects.create(
        creditor=creditor2, date=date(2024, 6, 1), number="INV-2024-102", status="paid"
    )
