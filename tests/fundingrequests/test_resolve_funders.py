...
def a_funder_match(*, name: str, links: tuple[Link, ...] = ()) -> FunderMatch:
    return FunderMatch(name=name, links=links)
...
def test__resolve_funders__crossref_id_matches_org_with_different_name() -> None:
    ...
    result = resolve_funders(
        [
            a_funder_match(
                name="Ministerio de Economía y Competitividad", links=(crossref_link("100000014"),)
            )
        ]
    )
...
def test__resolve_funders__duplicate_doi_different_names__does_not_raise_keyerror() -> None:
    ...
    When multiple FunderMatch objects share the same identifier, the matched
    org is resolved for each by its own name.
    """

    result = resolve_funders(
        [
            FunderMatch(name="NSFC", links=(doi_link("10.13039/501100001809"),)),
            FunderMatch(
...
def test_resolve_funders_same_funder_repeated_in_batch_creates_one_link_per_type() -> None:
    ...
    org = FundingOrganization.objects.get(name="Funder A")
...
def test_resolve_funders_doi_only_funder_repeated_creates_single_doi_link() -> None:
    resolve_funders(
        [
            a_funder_match(name="Funder A", links=(doi_link("10.13039/501100004663"),)),
            a_funder_match(name="Funder A", links=(doi_link("10.13039/501100004663"),)),
            a_funder_match(name="Funder A", links=(doi_link("10.13039/501100004663"),)),
        ]
    )
...
def test_resolve_funders_crossref_only_funder_repeated_creates_single_crossref_link() -> None:
    resolve_funders(
        [
            a_funder_match(name="Funder A", links=(crossref_link("501100004663"),)),
            a_funder_match(name="Funder A", links=(crossref_link("501100004663"),)),
            a_funder_match(name="Funder A", links=(crossref_link("501100004663"),)),
        ]
    )
...
def test_resolve_funders_link_already_in_db_is_not_duplicated() -> None:
    crossref_type = FundingOrganizationLinkType.objects.get(name="Crossref")
    org = FundingOrganization.objects.create(name="Existing Org")
    FundingOrganizationLink.objects.create(
        type=crossref_type, value="100000014", funding_organization=org
    )

    resolve_funders([a_funder_match(name="Other Name", links=(crossref_link("100000014"),))])
...
def test_resolve_funders_ror_link_is_persisted() -> None:
    """Given a FunderMatch carrying a ROR link, the ROR ID is persisted as a link.
    ...
    resolve_funders([a_funder_match(name="Funder A", links=(Ror("https://ror.org/01pp8nd67"),))])
