...
from .decorators import (
    build_breadcrumb_url,
    get_preserved_query_params,
    extract_filters_from_referer,
)
from .view_resolver import resolve_breadcrumb_metadata


def _build_breadcrumb_hierarchy(
    url_name: str, request: HttpRequest, query_params: str = "", **original_kwargs: Any
) -> list[dict[str, Any]]:
    """
    ...
    while current_url_name and current_url_name not in visited:
        ...
        try:
            ...
            breadcrumbs.insert(0, {"title": title, "url": breadcrumb_url, "is_current": False})
...
def breadcrumb_context(request: HttpRequest) -> dict[str, Any]:
    ...
    if parent_url_name:
        ...
        breadcrumbs = _build_breadcrumb_hierarchy(
            parent_url_name, request, query_params, **current_kwargs
        )
...
def _add_home_breadcrumb(
    request: HttpRequest, breadcrumbs: list[dict[str, Any]], current_page_title: str
) -> list[dict[str, Any]]:
