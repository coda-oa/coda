...
from coda.domain.invoice import FundingSourceId, Position, PublicationCostType, TaxRate
from coda.domain.money import Currency, Money
from coda.domain.publication.publication import PublicationId


def to_position(position: PublicationPositionDto, currency: Currency) -> Position[PublicationId]:
    return Position(
        item=PublicationId(position.id),
...
def position_to_dto(position: Position[PublicationId]) -> PublicationPositionDto:
    ...
    reference = repository.get_reference_by_publication(publication.id)
    if reference:
